...
parser.add_argument('--skip_row', type=int,
                    help="Rows to skip, Zero indexed")
...
cc = pd.read_excel(
    sys.argv[1], skiprows=args.skip_row,
    header=0, parse_dates=[args.date_col])
            ...
            for i, r in cc.iterrows():
                if not pd.isnull(r[0]):
                    ...
                    of.write(
                        f'{txn_date} * " {r[3]}"\n')
                    if r[0] == 'i':
                        of.write(f'\tExpenses:INR:Internet {r[6]}\n')
                    elif r[0] == 'm':
                        of.write(f'\tExpenses:INR:Phone {r[6]}\n')
                    else:
                        of.write(f'\tExpenses:INR:Misc {r[6]}\n')
    ...
    if args.bank == 'idfc':
        if args.type == 'bank':
            ...
            if not pd.isnull(r[4]):  # Debit
                ...
                of.write(f'\tAssets:INR:AccountReceivable -{r[6]}\n')
